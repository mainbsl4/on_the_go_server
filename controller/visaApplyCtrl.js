...
const updateVisaAppStatus = asyncHandler(async (req, res) => {
  ...
  const { buyingPrise, sellingPrise, trackingId, comment, deliveredVisa, applicationCopy, paymentReceive } = req.body;


  // Validate input
  const validStatuses = ['SUBMITTED', 'CANCELLED', 'RECEIVED', 'APPLIED', 'APPROVED', 'REJECTED', "DELIVERED"];
  ...
  try {
    ...
    const updatedVisaApply = await prisma.visa_Apply.update({
      where: { id },
      data: { isApproved: status, comment: comment || null, buyingPrise: buyingPrise || null, sellingPrise: sellingPrise || null, deliveredVisa: deliveredVisa || null, applicationCopy: applicationCopy  || null, paymentReceive: paymentReceive || null, trackingId: trackingId || null },
    });
...
const updateVisaApply = asyncHandler(async (req, res) => {
  ...
  if (passExpiryDate !== undefined) visaDataToUpdate.passExpiryDate = passExpiryDate;
  ...
  if (applyForCountry !== undefined) visaDataToUpdate.applyForCountry = applyForCountry;
  if (deliveredVisa !== undefined) visaDataToUpdate.deliveredVisa = deliveredVisa;
  if (applicationCopy !== undefined) visaDataToUpdate.applicationCopy = applicationCopy;
  if (paymentReceive !== undefined) visaDataToUpdate.paymentReceive = paymentReceive;
  ...
  if (otherDocumentPdf !== undefined) visaDataToUpdate.otherDocumentPdf = otherDocumentPdf;
  if (previousPassPdf !== undefined) visaDataToUpdate.previousPassPdf = previousPassPdf;
...
const deleteVisaApp = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.json({ status: 200, message: 'Visa Apply deleted successfully' });
  } catch (error) {
